...
//     let priv_keys = values_t!(m.values_of("private_keys"),
// String).unwrap_or_default();     let len = priv_keys.len();
//     if len > number {
...
//     } else {
//         String::from_utf8(
//             hex::decode(common_ref_encoded).expect("common_ref should be a
// hex string"),         )
...
//         let seckey = if i < len {
//             Bytes::from(hex::decode(&priv_keys[i]).expect("decode hex private
// key"))         } else {
//             let mut seed = [0u8; 32];
...
//         let address =
// Address::from_pubkey_bytes(pubkey.clone()).expect("address");
...
//         let priv_key =
//             BlsPrivateKey::try_from([&[0u8; 16],
// seckey.as_ref()].concat().as_ref()).unwrap();         let pub_key =
// priv_key.pub_key(&common_ref.as_str().into());         k.bls_public_key =
// add_0x(hex::encode(pub_key.to_bytes()));         k.index = i + 1;
