...
// use overlord::types::{AggregatedSignature, AggregatedVote, Node, SignedVote,
// Vote, VoteType}; use overlord::{extract_voters, Crypto};
// use parking_lot::RwLock;

// use common_crypto::{
//     BlsCommonReference, BlsPrivateKey, BlsPublicKey, HashValue, PrivateKey,
// PublicKey,     Secp256k1PrivateKey, Secp256k1PublicKey, Signature,
// ToPublicKey, };
// use common_merkle::Merkle;

// use protocol::codec::ProtocolCodec;
// use protocol::traits::{CommonConsensusAdapter, Synchronization,
// SynchronizationAdapter}; use protocol::traits::{Context, Executor,
// ExecutorResp, ServiceResponse, TrustFeedback}; use protocol::types::{
//     Address, Block, Header, Bytes, Hash, Hex, MerkleRoot, Metadata, Proof,
// RawTransaction,     Receipt, ExecResp, SignedTransaction, TransactionRequest,
...
// use crate::util::{convert_hex_to_bls_pubkeys, digest_signed_transactions,
// OverlordCrypto}; use crate::BlockHeaderField::{PreviousBlockHash, ProofHash,
// Proposer}; use crate::BlockProofField::{BitMap, HashMismatch, HeightMismatch,
// WeightNotFound}; use crate::{BlockHeaderField, BlockProofField,
// ConsensusError};

// const PUB_KEY_STR: &str =
// "02ef0cb0d7bc6c18b4bea1f5908d9106522b35ab3c399369605d4242525bda7e60";
...
//         let remote_blocks =
// gen_remote_block_hashmap(list_rich_block.0.clone());         let
// remote_proofs = gen_remote_proof_hashmap(list_rich_block.1.clone());
...
// gen_remote_tx_hashmap(list_rich_block.0.clone());
...
//         ));
//         let block_hash =
// Hash::digest(genesis_block.header.encode_fixed().unwrap());         let
// status = CurrentConsensusStatus {             cycles_price:                1,
...
// genesis_block.header.state_root.clone(),             list_state_root:
// vec![],             list_receipt_root:           vec![],
...
//         block_on(sync.receive_remote_block(Context::new(), max_height /
// 2)).unwrap();

//         // get the current consensus status to check if the test works fine
//         let status = status_agent.to_inner();
//         let block =
//             block_on(adapter.get_block_by_height(Context::new(),
// status.latest_committed_height))                 .unwrap();
//         assert_sync(status, block);

//         block_on(sync.receive_remote_block(Context::new(),
// max_height)).unwrap();         let status = status_agent.to_inner();
//         let block =
//             block_on(adapter.get_block_by_height(Context::new(),
// status.latest_committed_height))                 .unwrap();
//         assert_sync(status, block);
...
//     async fn get_block_from_remote(&self, _: Context, height: u64) ->
// ProtocolResult<Block> {         Ok(self.remote_blocks.read().get(&height).
// unwrap().clone())     }
...
//     async fn get_proof_from_remote(&self, _: Context, height: u64) ->
// ProtocolResult<Proof> {         Ok(self.remote_proofs.read().get(&height).
// unwrap().clone())     }
// }
...
//     async fn save_block(&self, _: Context, block: Block) ->
// ProtocolResult<()> {         self.local_blocks.write().insert(block.header.
// height, block);         let mut height = self.latest_height.write();
//         *height += 1;
//         Ok(())
//     }
...
//     async fn save_receipts(&self, _: Context, _: u64, _: Vec<Receipt>) ->
// ProtocolResult<()> {         Ok(())
//     }

//     /// Flush the given transactions in the mempool.
//     async fn flush_mempool(&self, _: Context, _: &[Hash]) ->
// ProtocolResult<()> {         Ok(())
//     }

//     /// Get a block corresponding to the given height.
//     async fn get_block_by_height(&self, _: Context, height: u64) ->
// ProtocolResult<Block> {         Ok(self.local_blocks.read().get(&height).
// unwrap().clone())     }
...
//     async fn broadcast_height(&self, _: Context, _: u64) ->
// ProtocolResult<()> {         Ok(())
//     }
...
//     async fn verify_block_header(&self, ctx: Context, block: &Block) ->
// ProtocolResult<()> {         let previous_block = self
//             .get_block_by_height(ctx.clone(), block.header.height - 1)
//             .await?;

//         let previous_block_hash =
// Hash::digest(previous_block.header.encode_fixed()?);

//         if previous_block_hash != block.header.prev_hash {
//             log::error!(
//                 "[consensus] verify_block_header, previous_block_hash: {:?},
// block.header.prev_hash: {:?}",                 previous_block_hash,
//                 block.header.prev_hash
//             );
//             return Err(
//                 ConsensusError::VerifyBlockHeader(block.header.height,
// PreviousBlockHash).into(),             );
//         }

//         // the block 0 and 1 's proof is consensus-ed by community
//         if block.header.height > 1u64 && block.header.prev_hash !=
// block.header.proof.block_hash {             log::error!(
//                 "[consensus] verify_block_header, verifying_block : {:?}",
//                 block
//             );
//             return Err(ConsensusError::VerifyBlockHeader(block.header.height,
// ProofHash).into());         }
...
//         {
//             log::error!(
//                 "[consensus] verify_block_header, block.header.proposer:
// {:?}, authority_map: {:?}",                 block.header.proposer,
//                 authority_map
//             );
//             return Err(ConsensusError::VerifyBlockHeader(block.header.height,
// Proposer).into());         }
...
// Address::from_pubkey_bytes(validator.pub_key.clone());

//             if !authority_map.contains_key(&validator.pub_key) {
//                 log::error!(
//                     "[consensus] verify_block_header, validator.address:
// {:?}, authority_map: {:?}",                     validator_address,
//                     authority_map
//                 );
...
//         proof: &Proof,
//     ) -> ProtocolResult<()> {
//         // the block 0 has no proof, which is consensus-ed by community, not
// by chain
...
//             log::error!(
//                 "[consensus] verify_proof, block_header.height: {},
// proof.height: {}",                 block_header.height,
//                 proof.height
//             );
...
//                 proof.block_hash
//             );
//             return Err(ConsensusError::VerifyProof(block_header.height,
// HashMismatch).into());         }
...
//         let signed_voters = extract_voters(&mut authority_list,
// &proof.bitmap).map_err(|_| {             log::error!("[consensus]
// extract_voters fails, bitmap error");
...
//         let vote_hash =
// self.crypto.hash(protocol::Bytes::from(rlp::encode(&vote)));         let
// hex_pubkeys = metadata             .verifier_list
//             .iter()
//             .filter_map(|v| {
...
//             hex_pubkeys,
//         ).map_err(|e| {
//             log::error!("[consensus] verify_proof_signature error, height {},
// vote: {:?}, vote_hash:{:?}, sig:{:?}, signed_voter:{:?}",
// block_header.height,             vote,
//             vote_hash,
//             proof.signature,
//             signed_voters,
//             );
...
//         self.verify_proof_weight(ctx.clone(), block_header.height,
// weight_map, signed_voters)?;
...
//         self.crypto
//             .inner_verify_aggregated_signature(vote_hash, pub_keys,
// aggregated_signature_bytes)             .map_err(|e| {
//                 log::error!("[consensus] verify_proof_signature error: {}",
// e);                 ConsensusError::VerifyProof(block_height,
// BlockProofField::Signature).into()             })
//     }
...
//     ) -> ProtocolResult<()> {
//         let total_validator_weight: u64 = weight_map.iter().map(|pair|
// u64::from(*pair.1)).sum();
...
//                 let weight =
// weight_map.get(signed_voter_address).ok_or_else(|| {
// log::error!(                         "[consensus] verify_proof_weight,
...
//                 return Err(
//                     ConsensusError::VerifyProof(block_height,
// BlockProofField::Validator).into(),                 );
//             }
//         }
...
//             return Err(ConsensusError::VerifyProof(block_height,
// BlockProofField::Weight).into());         }
//         Ok(())
//     }
// }
...
//     let priv_key =
// BlsPrivateKey::try_from(hex::decode("
// 00000000000000000000000000000000d654c7a6747fc2e34808c1ebb1510bfb19b443d639f2fab6dc41fce9f634de37"
// ).unwrap().as_ref()).unwrap();     OverlordCrypto::new(priv_key,
// HashMap::new(), "muta".into()) }

// fn gen_remote_tx_hashmap(list: Vec<RichBlock>) -> SafeHashMap<Hash,
// SignedTransaction> {     let mut remote_txs = HashMap::new();
...
//     for rich_block in list.into_iter() {
//         remote_blocks.insert(rich_block.block.header.height,
...
// fn mock_chained_rich_block(len: u64, gap: u64, key_tool: &KeyTool) ->
// (Vec<RichBlock>, Vec<Proof>) {     let mut list_rich_block = vec![];
...
//     loop {
//         let last_block_hash =
// Hash::digest(last_rich_block.block.header.encode_fixed().unwrap());
//         let last_header = &last_rich_block.block.header;

//         let txs = mock_tx_list(3, current_height);
//         let tx_hashes: Vec<Hash> = txs.iter().map(|tx|
// tx.tx_hash.clone()).collect();         let order_root =
// Merkle::from_hashes(tx_hashes.clone())             .get_root_hash()
//             .unwrap();
//         let order_signed_transactions_hash =
// digest_signed_transactions(&txs).unwrap();
...
//                 let confirm_root =
// rich_block.block.header.order_root.clone();                 let (exec_resp,
// receipt_root) = exec_txs(height, &rich_block.txs);
...
// Hash::digest(rich_block.block.header.encode_fixed().unwrap());

//         // generate proof for current height and for next block use
//         last_proof = mock_proof(current_block_hash.clone(), current_height,
// 0, &key_tool);
...
//         cycles_used:                    vec![],
//         proposer:
// "muta14e0lmgck835vm2dfm0w3ckv6svmez8fdgdl705"             .parse()
//             .unwrap(),
//         proof:                          Proof {
//             height:     0,
//             round:      0,
...
//                 .unwrap();
//         let test_privkey =
// Secp256k1PrivateKey::try_from(hex_privkey.as_ref()).unwrap();         let
// test_pubkey = test_privkey.pub_key();         let _test_address =
// Address::from_pubkey_bytes(test_pubkey.to_bytes()).unwrap();
...
// // Vote!!!!!!!
// fn mock_proof(block_hash: Hash, height: u64, round: u64, key_tool: &KeyTool)
// -> Proof {     let vote = Vote {
//         height,
//         round,
//         vote_type: VoteType::Precommit,
...
//         signature:      key_tool
//             .overlord_crypto
//             .aggregate_signatures(vec![signed_vote.signature],
// vec![signed_vote.voter])             .unwrap(),
...
// fn exec_txs(height: u64, txs: &[SignedTransaction]) -> (ExecutorResp,
// MerkleRoot) {     let mut receipts = vec![];
...
// impl SignerNode {
//     pub fn new(secp_private_key: Secp256k1PrivateKey, secp_public_key:
// Secp256k1PublicKey) -> Self {         SignerNode {
//             secp_private_key,
//             secp_public_key,
//         }
//     }
...
//     let bls_priv_key =
// BlsPrivateKey::try_from(bls_priv_key.as_ref()).unwrap();

//     let (bls_pub_keys, common_ref) = get_mock_public_keys_and_common_ref();

//     let mock_crypto = OverlordCrypto::new(bls_priv_key, bls_pub_keys,
// common_ref);

//     KeyTool::new(signer_node, Arc::new(mock_crypto), mock_verifier_list())
// }

// fn get_mock_public_keys_and_common_ref() -> (HashMap<Bytes, BlsPublicKey>,
// BlsCommonReference) {     let mut bls_pub_keys: HashMap<Bytes, BlsPublicKey>
// = HashMap::new();

//     // weight = 5
//     let bls_hex =
// Hex::from_string("
// 0x04102947214862a503c73904deb5818298a186d68c7907bb609583192a7de6331493835e5b8281f4d9ee705537c0e765580e06f86ddce5867812fceb42eecefd209f0eddd0389d6b7b0100f00fb119ef9ab23826c6ea09aadcc76fa6cea6a32724"
// .to_string()     ).unwrap();
...
//     let bls_hex =
// Hex::from_string("
// 0x0418e16bd67ce0b58a575f506967706be733c96feef19a06bb37d510000d89905f2f61b7da4d831cb1bb01e2f99833362602a0a252dfd1e95c75c1eadb0db220e3722c9a077b730e7f6cec5f4a55bfc9a4d88db3e6c27684aa8335456824070501"
// .to_string()     ).unwrap();
//     let bls_hex = hex::decode(bls_hex.as_string_trim0x()).unwrap();
//     bls_pub_keys.insert(
//         Hex::from_string(
//
// "0x03dbd1dbf3835efb4ec34a360ee671ee1d22425425368edfc5b9ffafc812e86200".
// to_owned(),         )
//         .unwrap()
//         .decode(),
//         BlsPublicKey::try_from(bls_hex.as_ref()).unwrap(),
...
//     let bls_hex =
// Hex::from_string("
// 0x040944276f414c46330227f2c0c5a998aba3d400ed19cfc2d31d3e7fcc442ce9f91ea86e172dc3c1b6cedc364bd52ba1cf074529e52337cd80ab32a196a3d42ab46eee25120b44fdd2b5c4268bf3b84c72d068ea83d0530a5461dc30b6a63a60e9"
// .to_string()     ).unwrap();
//     let bls_hex = hex::decode(bls_hex.as_string_trim0x()).unwrap();
//     bls_pub_keys.insert(
//         Hex::from_string(
//
// "0x03cba4ae147eb24891d78c9527798577419b7db913b4b03ba548c28f40c5841166".
// to_owned(),         )
//         .unwrap()
//         .decode(),
//         BlsPublicKey::try_from(bls_hex.as_ref()).unwrap(),
...
// Hex::from_string("
// 0x04102947214862a503c73904deb5818298a186d68c7907bb609583192a7de6331493835e5b8281f4d9ee705537c0e765580e06f86ddce5867812fceb42eecefd209f0eddd0389d6b7b0100f00fb119ef9ab23826c6ea09aadcc76fa6cea6a32724"
// .to_owned()).unwrap(),             pub_key:
// Hex::from_string("
// 0x02ef0cb0d7bc6c18b4bea1f5908d9106522b35ab3c399369605d4242525bda7e60".
// to_owned()).unwrap(),             address:
// Address::from_str("muta14e0lmgck835vm2dfm0w3ckv6svmez8fdgdl705").unwrap(),
//             propose_weight: 5,
//             vote_weight:    5,
//         },
...
// 0x03cba4ae147eb24891d78c9527798577419b7db913b4b03ba548c28f40c5841166".
// to_owned()).unwrap(),             address:
// Address::from_str("muta1h99h6f54vytatam3ckftrmvcdpn4jlmnwm6hl0").unwrap(),
//             propose_weight: 1,
//             vote_weight:    1,
//         },
//     ]
...
// "public_key":
// "0x02ef0cb0d7bc6c18b4bea1f5908d9106522b35ab3c399369605d4242525bda7e60", //
// "address": "muta14e0lmgck835vm2dfm0w3ckv6svmez8fdgdl705", //       "peer_id":
// "QmTEJkB5QKWsEq37huryZZfVvqBKb54sHnKn9TQcA6j3n9", //       "bls_public_key":
// "0x04102947214862a503c73904deb5818298a186d68c7907bb609583192a7de6331493835e5b8281f4d9ee705537c0e765580e06f86ddce5867812fceb42eecefd209f0eddd0389d6b7b0100f00fb119ef9ab23826c6ea09aadcc76fa6cea6a32724"
// //     },
// //     {
// //       "index": 2,
...
// "public_key":
// "0x03dbd1dbf3835efb4ec34a360ee671ee1d22425425368edfc5b9ffafc812e86200", //
// "address": "muta15a8a9ksxe3hhjpw3l7wz7ry778qg8h9wz8y35p", //       "peer_id":
// "QmaEX2TxiC2YJufqcHRigVpnoxahX3hdR1gsFjD5Yf7K1Z", //       "bls_public_key":
// "0x0418e16bd67ce0b58a575f506967706be733c96feef19a06bb37d510000d89905f2f61b7da4d831cb1bb01e2f99833362602a0a252dfd1e95c75c1eadb0db220e3722c9a077b730e7f6cec5f4a55bfc9a4d88db3e6c27684aa8335456824070501"
// //     },
// //     {
// //       "index": 3,
...
// "public_key":
// "0x03cba4ae147eb24891d78c9527798577419b7db913b4b03ba548c28f40c5841166", //
// "address": "muta1h99h6f54vytatam3ckftrmvcdpn4jlmnwm6hl0", //       "peer_id":
// "QmbRmcYD3j2zMr27C6Ga2Bo5xB9t37NyAt36cSvUGYXE2B", //       "bls_public_key":
// "0x040944276f414c46330227f2c0c5a998aba3d400ed19cfc2d31d3e7fcc442ce9f91ea86e172dc3c1b6cedc364bd52ba1cf074529e52337cd80ab32a196a3d42ab46eee25120b44fdd2b5c4268bf3b84c72d068ea83d0530a5461dc30b6a63a60e9"
// //     },
// //     {
// //       "index": 4,
...
// "public_key":
// "0x0245a0c291f56c2c5751db1c0bf1ed986e703d29a0fe023df770fe92c7c2347316", //
// "address": "muta16xukzz73l5r6vulk9q697tave8c5mfu33mwud6", //       "peer_id":
// "QmeqYprgrXwxzLP7qAFiiJ3Kfi3F6H9PPH2qPCEHr9cRYW", //       "bls_public_key":
// "0x041342e9a35278b298a67006cd98d663053e3f7eb72a08ffe9835074e430b2112a866c1c8d981edcd793cb16d459fc952b0464007d876355eea671e74727588bae69740c6a0b49d8142b7b0821a78acd34b4d8012b9ef69444a476e03d5fea5330"
// //     }
// //   ]
// // }

// fn assert_sync(status: CurrentConsensusStatus, latest_block: Block) {
//     let exec_gap = latest_block.header.height -
// latest_block.header.exec_height;
