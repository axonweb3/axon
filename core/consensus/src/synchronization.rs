...
#[cfg(test)]
mod tests {
    use crate::tests::synchronization::MockSyncAdapter;
    use crate::util::time_now;
    use protocol::tokio::sync::Mutex as AsyncMutex;
    use protocol::ProtocolError;
    use protocol::{tokio, types::Header};
    ...
    async fn test_verify_block() {
        ...
        //     Err(err) => {
        //         assert_eq!(err,
        // (Err(ConsensusError::InvalidOrderSignedTransactionsHash)).into());
        //     }
        // }
        // assert_eq!(result,
        // (Err(ConsensusError::InvalidOrderSignedTransactionsHash)).into());
        // let x = Ok(Proof::default());
        // assert_eq!(result.unwrap(), Proof::default());
    }
}
