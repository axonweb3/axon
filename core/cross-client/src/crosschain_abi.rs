...
mod crosschain_mod {
    ...
    impl<'a, M: ethers_providers::Middleware> crosschain<M> {
        ...
        #[doc = "Calls the contract's `crossFromCKB` (0xbe52c515) function"]
        pub fn cross_from_ckb(
            &self,
            records: ::std::vec::Vec<CkbtoAxonRecord>,
            signatures: ethers_core::types::Bytes,
            nonce: ethers_core::types::U256,
        ) -> ethers_contract::builders::ContractCall<M, ()> {
            self.0
                .method_hash([190, 82, 197, 21], (records, signatures, nonce))
                .expect("method not found (this should never happen)")
        }
    ...
    #[doc = "Container type for all input parameters for the `crossFromCKB`function with signature `crossFromCKB((address,address,uint256,uint256,bytes32)[],bytes,uint256)` and selector `[190, 82, 197, 21]`"]
    #[derive(
        Clone,
        Debug,
        Default,
    ...
    #[ethcall(
        name = "crossFromCKB",
        abi = "crossFromCKB((address,address,uint256,uint256,bytes32)[],bytes,uint256)"
    )]
    pub struct CrossFromCKBCall {
        pub records:    ::std::vec::Vec<CkbtoAxonRecord>,
        pub signatures: ethers_core::types::Bytes,
