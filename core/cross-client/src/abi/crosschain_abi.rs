...
pub mod crosschain_mod {
    ...
    impl<M: ethers_providers::Middleware> crosschain<M> {
        ...
        #[doc = "Calls the contract's `approveLimitTx` (0x78196bd6) function"]
        pub fn approve_limit_tx(
            &self,
            record: AxonToCKBRecord,
        ) -> ethers_contract::builders::ContractCall<M, ()> {
            self.0
                .method_hash([120, 25, 107, 214], (record,))
                .expect("method not found (this should never happen)")
        }
    ...
    #[ethcall(
        name = "approveLimitTx",
        abi = "approveLimitTx((address,uint256,uint256,string,uint256))"
    )]
    ...
    pub enum crosschainCalls {
        ...
        ApproveLimitTx(ApproveLimitTxCall),
    ...
    impl ethers_core::abi::AbiDecode for crosschainCalls {
        fn decode(data: impl AsRef<[u8]>) -> Result<Self, ethers_core::abi::AbiError> {
            ...
            if let Ok(decoded) =
                <ApproveLimitTxCall as ethers_core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(crosschainCalls::ApproveLimitTx(decoded));
            }
            if let Ok(decoded) =
    ...
    impl ethers_core::abi::AbiEncode for crosschainCalls {
        fn encode(self) -> Vec<u8> {
            match self {
                ...
                crosschainCalls::ApproveLimitTx(element) => element.encode(),
    ...
    impl ::std::fmt::Display for crosschainCalls {
        fn fmt(&self, f: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
            match self {
                ...
                crosschainCalls::ApproveLimitTx(element) => element.fmt(f),
    ...
    impl ::std::convert::From<ApproveLimitTxCall> for crosschainCalls {
        fn from(var: ApproveLimitTxCall) -> Self {
            crosschainCalls::ApproveLimitTx(var)
        }
    }
