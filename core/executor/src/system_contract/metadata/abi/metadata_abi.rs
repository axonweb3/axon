...
pub mod metadata_contract {
    ...
    impl<M: ethers::providers::Middleware> MetadataContract<M> {
        ...
        pub fn new<T: Into<ethers::core::types::Address>>(
            ...
            ethers::contract::Contract::new(address.into(), METADATACONTRACT_ABI.clone(), client)
                .into()
        }
    ...
    impl<M: ethers::providers::Middleware> From<ethers::contract::Contract<M>> for MetadataContract<M> {
    ...
    impl ethers::core::abi::AbiDecode for MetadataContractCalls {
        fn decode(data: impl AsRef<[u8]>) -> Result<Self, ethers::core::abi::AbiError> {
            ...
            if let Ok(decoded) =
                <IsProposerCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(MetadataContractCalls::IsProposer(decoded));
            }
            if let Ok(decoded) =
                <IsVerifierCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
                return Ok(MetadataContractCalls::IsVerifier(decoded));
            }
            if let Ok(decoded) =
                <VerifierListCall as ethers::core::abi::AbiDecode>::decode(data.as_ref())
            {
